...
function enviarRespuesta($success, $ruta = '', $error = '') {
    echo json_encode([
        'success' => $success,
        'ruta' => $ruta,
        'error' => $error
...
try {
    ...
    $brillo = isset($ajustes['brillo']) ? (int)$ajustes['brillo'] : 0;
    $contraste = isset($ajustes['contraste']) ? (int)$ajustes['contraste'] : 0;
    $saturacion = isset($ajustes['saturacion']) ? (int)$ajustes['saturacion'] : 0;
    $exposicion = isset($ajustes['exposicion']) ? (int)$ajustes['exposicion'] : 0;
    ...
    $temperatura = isset($ajustes['temperatura']) ? (int)$ajustes['temperatura'] : 0;
    if ($temperatura != 0) {
    ...
    // Tinte
    $tinte = isset($ajustes['tinte']) ? (int)$ajustes['tinte'] : 0;
    if ($tinte != 0) {
    ...
    // Desenfoque
    $desenfoque = isset($ajustes['desenfoque']) ? (int)$ajustes['desenfoque'] : 0;
    if ($desenfoque > 0) {
    ...
    // Nitidez
    $nitidez = isset($ajustes['nitidez']) ? (int)$ajustes['nitidez'] : 0;
    if ($nitidez > 0) {
        $matrix = array(
            array(-1, -1, -1),
    ...
    $filtro = isset($ajustes['filtro']) ? $ajustes['filtro'] : 'ninguno';
    ...
    // Guardar imagen final con timestamp único
    $nombreFinal = 'uploads/editada_' . time() . '_' . uniqid() . '.jpg';
    
    if (!imagejpeg($imagen, $nombreFinal, 95)) {
        imagedestroy($imagen);
        enviarRespuesta(false, '', 'Error al guardar la imagen');
    }
    
    imagedestroy($imagen);

    // Respuesta exitosa
    enviarRespuesta(true, $nombreFinal, '');

} catch (Exception $e) {
    enviarRespuesta(false, '', 'Error interno: ' . $e->getMessage());
} catch (Error $e) {
    enviarRespuesta(false, '', 'Error fatal: ' . $e->getMessage());
}
?>